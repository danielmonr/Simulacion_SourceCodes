...
double calc(double ui){
  return (ui < 0.5)? sqrt(2*ui): 2-sqrt(2-2*ui);
}
...
void process(){
  ...
  while(getline(r_file, line)){
    ui = read(line);
    w_file << cont << "," << ui << "," << calc(ui) << "\n";
    vals.push_back(calc(ui));
    cont++;
  }
  cout << "N= " << cont-1 << endl;
}
...
int main(int argc, char* argv[]){
  ...
  w_file.open("triang.csv");
  w_file2.open("triang_classes.csv");
